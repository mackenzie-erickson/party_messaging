...
estimateNetwork_outputEigens.fns <- function(first.obs){
  ...
  for (i in unique(first.obs$sessionStartDate)) {
    ...
    # Pull out nodes
    nodes <- sort(unique(c(session.netinf.result$origin_node, session.netinf.result$destination_node)))
    ...
    edge_list_REVERSE <- as.matrix(cbind(
      match(session.netinf.result$destination_node, nodes),
      match(session.netinf.result$origin_node, nodes)))
    ...
    # Calculate pagerank ('normal')
    pagerank_NORMAL <- page_rank(g_NORMAL)
    # Calculate pagerank ('reverse')
    pagerank_REVERSE <- page_rank(g_REVERSE)
    ...
    degree_data <- 
      merge(
        as.data.table(edge_list_REVERSE)[, .N, V1][, .(V = V1, destination_degree = N)],
        as.data.table(edge_list_REVERSE)[, .N, V2][, .(V = V2, origin_degree = N)],
        by = "V", all = TRUE
      )
    
    # Add pagerank_REVERSE to dt
    degree_data[, pagerank_REVERSE := pagerank_REVERSE$vector]
    # Add pagerank_NORMAL
    degree_data[, pagerank_NORMAL := pagerank_NORMAL$vector]
    # Calculate diff in pagerank between REVERSE - NORMAL
    degree_data[, diff := pagerank_REVERSE - pagerank_NORMAL]
